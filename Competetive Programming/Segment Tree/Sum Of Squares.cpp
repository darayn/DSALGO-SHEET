...
void lazy_update_incrementor(tree_attrs* tree,lazy_attrs* lazy, int start, int end, int left, int right, int current_position, int increment){
	...
	if(lazy[current_position].incrementor!=0){
		tree[current_position].sum_of_squares +=(right-left+1) * (lazy[current_position].incrementor *lazy[current_position].incrementor ) + 2 * lazy[current_position].incrementor*tree[current_position].sum;
		tree[current_position].sum += (right-left+1) * lazy[current_position].incrementor;
		if(start!=end){
			lazy[2*current_position].incrementor += lazy[current_position].incrementor;
			lazy[2*current_position+1].incrementor +=lazy[current_position].incrementor;
		}
		lazy[current_position].incrementor=0;
	}
	if (lazy[current_position].setter != 0)
	{
		tree[current_position].sum = (right - left + 1) * (lazy[current_position].setter);
	...
	if(start>=left&&end<=right)
	{
		...
		if (start != end)
		{
			lazy[2 * current_position].incrementor += increment;
			lazy[2 * current_position + 1].incrementor += increment;
		}
		return;
	}
	//partial overlap
