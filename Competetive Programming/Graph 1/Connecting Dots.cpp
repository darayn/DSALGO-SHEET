...
#include<bits/stdc++.h>
void dfs(vector<vector<char>> &board, vector<vector<bool>> &visited, int x, int y, int fromX,int fromY, char color, bool &foundCycle, int n, int m){
	if(x<0 || x>=n || y<0 || y>=m){
		return;
	}
	if(board[x][y]!=color){
	...
	for(int i=0;i<4;i++){
		...
		dfs(board, visited, nextX, nextY, x,y, color, foundCycle,n, m);
	}
}
