...
// Different techniques are discussed above 
// I will be solving end to end problem below;

typedef long long int ll;
ll m = 1000000000 + 7;


ll alphaCodes(ll *arr, ll *output, ll size){
	if(size ==0 || size == 1){
		return 1;
	}
	ll count = 0;
	...
	if(arr[size-1]!=0){
		count += alphaCodes(arr,output,size-1)%m;
	}
	if(arr[size-2]*10+arr[size-1] <=26 && arr[size-2]!=0){
		count += alphaCodes(arr, output,size-2)%m;
		count %= m;
	}
	output[size] = count;
	return count;
...
int main(){
	while(true){
		...
		if(s == "0"){
			...
			ll* arr = new ll[s.size() + 1];
			for(ll i=0;i<s.size();i++){
				arr[i] = s[i] - '0';
				//converting string elements to int as input in array
			}
			cout<<alphaCodes(arr, output ,s.size())<<endl;
			delete [] arr;
			delete [] output;
		}
