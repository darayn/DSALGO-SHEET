...
int number_of_possible_brackets_expressions(bool * boolean_arr, int n, int opening_till_now, int closing_till_now, int **dp){
	if(boolean_arr[0]){
		return  0;
	}
	if(dp[opening_till_now][closing_till_now]>=0){
		return dp[opening_till_now][closing_till_now];
	}
	if(opening_till_now >n || closing_till_now>n){
		return 0;
	}
	if(opening_till_now == n && closing_till_now == n){
		return 1;
	...
	int ans;
	if(opening_till_now==closing_till_now ||boolean_arr[opening_till_now+closing_till_now+1]){
		ans = number_of_possible_brackets_expressions(boolean_arr,n, opening_till_now+1,closing_till_now, dp);
	}
	else if(opening_till_now==n){
		ans = number_of_possible_brackets_expressions(boolean_arr,n, opening_till_now,closing_till_now+1, dp);
	}
	else{
		int option1 = number_of_possible_brackets_expressions(boolean_arr,n, opening_till_now+1,closing_till_now, dp);
		int option2 = number_of_possible_brackets_expressions(boolean_arr,n, opening_till_now,closing_till_now+1, dp);
		ans  = option1 + option2;
	}
	dp[opening_till_now][closing_till_now] = ans;
	return ans;
}
int main(){
	int t;
	...
	while(t--){
		...
		cout<<number_of_possible_brackets_expressions(boolean_arr,n,0,0,dp)<<endl;
	}
}
