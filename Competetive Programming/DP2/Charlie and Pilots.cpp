...
#include<bits/stdc++.h>
using namespace std;
int minimum_money(int *captain, int *assistant, int n, int x, int ** dp){
	if(n==0){
		return 0;
	}
	if(dp[n][x]>=0){
	...
	int ans;
	if(x==0){
		ans = assistant[0]+minimum_money(captain+1, assistant+1, n-1, x+1, dp);	
	}
	else if(x==n){
		ans = captain[0]+minimum_money(captain+1, assistant+1, n-1, x-1, dp);	
	}
	else{
		int option1 = assistant[0]+minimum_money(captain+1, assistant+1, n-1, x+1, dp);
		int option2 = captain[0]+minimum_money(captain+1, assistant+1, n-1, x-1, dp);
